...
export default function Request(data) {
  ...
  useEffect(() => {
    if (data.location.state.path.includes("EclipseAdmin")) {
      setNavType("EclipseAdmin");
      console.log();
    } else if (data.location.state.path.includes("EclipseManager")) {
      setNavType("EclipseManager");
    } else if (data.location.state.path.includes("TimAdmin")) {
      setNavType("TimAdmin");
    } else if (data.location.state.path.includes("TimManager")) {
      setNavType("TimManager");
    }
    if (data.location.state.path.includes("Pending")) {
      setTabType("Pending for Review");
    }
    if (data.location.state.path.includes("Request")) {
      setTabType("Request for Changes");
    }
    if (data.location.state.path.includes("Approved")) {
      setTabType("Approved");
    }
  }, []);
  ...
  return (
    <div className="container-fluid">
      <>
        {navType === "TimAdmin" ? <TimAdminNav /> : ""}
        {navType === "TimManager" ? <TimManagerNav heading={tabType} /> : ""}
        {navType === "EclipseManager" ? (
          <EclipseManagerNav heading={tabType} />
        ) : (
          ""
        )}
        {navType === "EclipseAdmin" ? <EclipseAdminNav /> : ""}

        <div class="cardd">
          <div class="card-header title-request p-0">
            <h3 class="card-title text-light px-5 pt-4 pb-3 bg-secondary">
              REQUEST
            </h3>
          </div>
              ...
              <div class="col-lg-12 col-9">
                <form class="text-left status-data-form">
                  <div class="row mt-4">
                    ...
                    <div class="col-md-6">
                      <b class="ml-3">
                        {data.location.state.requestData.Id ||
                          data.location.state.requestData.UserAccessRequestId}
                      </b>
                    </div>
                  </div>
                  <div class="row mt-4">
                    ...
                    <div class="col-md-6">
                      <b class="ml-3">
                        {data.location.state.requestData.FullName
                          ? data.location.state.requestData.FullName
                          : "Cunningham, John"}
                      </b>
                    </div>
                  ...
                  <div class="row mt-4">
                    <div class="col-md-6">
                      <NavLink
                        className="btn btn-secondary text-center shadow"
                        to={data.location.state.path}
                      >
                        Return To Records
                      </NavLink>
                    </div>
              ...
              <div class="accordion-wrapper">
                <div class="accordion">
                  ...
                  <div class="accordion-content">
                    ...
                    <div className="search">
                      ...
                      {filteredAccessData.length != 0 && (
                        <div className="dataResult">
                          {filteredAccessData.slice(0, 7).map((val, key) => {
                            return (
                              <span
                                onClick={() => selectAccess(val.Id)}
                    ...
                    <button
                      type="button"
                      class="btn btn-info change-access-btn color-white"
                    >
                      Submit
                    </button>
                  </div>
                ...
                <div class="accordion">
                  ...
                  <div class="accordion-content">
                    <div className="remove-access">
                      <button
                        type="button"
                        class="btn btn-info btn-lg color-white"
                      >
                        Remove Access
                      </button>
                    </div>
                ...
                <div class="accordion">
                  ...
                  <div class="accordion-content">
                    ...
                    <div className="search">
                      <div className="searchInputs">
                        <input
                          className="search-manager-input"
                          type="text"
                      ...
                      {filteredUserData.length != 0 && (
                        <div className="dataResult">
                          {filteredUserData.slice(0, 7).map((val, key) => {
                            return (
                              <span
                                onClick={() => selectManager(val.FullName)}
                                className="managerItem"
                              >
                                {val.FullName} &ensp;({val.Title})
                              </span>
                            );
                          })}
                        </div>
